function [] = joc()
    ...
    if isnan(nrPlayers) || ceil(nrPlayers) ~= nrPlayers || nrPlayers < 1 || nrPlayers > 2
        ...
        pPlayer = false(2, 1);
    elseif nrPlayers == 1
        firstPlayers = input('Would the human like to go first (Y/N)? ', 's');
        if length(firstPlayers) == 1 && lower(firstPlayers) == 'n'
            pPlayer = [false ; true];
        else
            pPlayer = [true ; false];
        end
    ...
    fprintf('So Player 1 is %shuman and %cs and Player 2 is %shuman and %cs.\n', ...
        char('not '.*~pPlayer(1)), marks(1), char('not '.*~pPlayer(2)), marks(2))
    if nrPlayers > 0
      ...
      while ~gameOver
          ...
          if isValid && ~isQuit
              ...
              if CheckWin(boards(:, :, turn))
                  ...
                  fprintf('\n Player 1 wins : %d\n',Player1Count);
                  fprintf('\n Player 2 wins : %d\n',Player2Count);
                  decision = input('Do you want to play again ?(Y/N) ', 's');
                  if(length(decision) == 1 && lower(decision) == 'n')
                    condition = 0;
                  end  
